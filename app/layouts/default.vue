...
<template>
  ...
  <div class="layout">
    ...
    <!-- 组件来源：Nuxt 自动导入 ~/components/app/AppHeader.vue（基于 components/ 目录约定） -->
    <AppHeader />
    <!-- 主内容区域，slot 插槽用于渲染页面级组件内容 -->
    <main class="layout__content">
      <!-- 默认插槽 slot：承接 Nuxt 路由匹配到的 pages/ 目录下的页面组件内容（由 <NuxtPage> 渲染后传入） -->
      <slot />
    </main>
    <!-- 页脚组件，包含版权信息等 -->
    <!-- 组件来源：Nuxt 自动导入 ~/components/app/AppFooter.vue（基于 components/ 目录约定） -->
    <AppFooter />
  </div>
</template>
...
.layout {
  ...
  display: flex;                  /* 弹性布局 */
  /* flex-direction: column 将 .layout 划分为纵向三个区域：① AppHeader（顶部导航）→ ② .layout__content（主内容，flex:1 填充）→ ③ AppFooter（页脚） */
...
.layout__content {
  ...
  /* padding-top 预留 64px：AppHeader 使用 fixed 固定定位后脱离文档流，若不预留空间，页面首屏内容会被导航栏遮挡（64px 与导航栏实际高度一致） */
