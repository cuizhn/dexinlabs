  ...
  <div class="admin-list">
    <div class="admin-list__head">
      <div>
        ...
        <h1 class="admin-list__title">练习管理 Exercise</h1>
      </div>
      <NuxtLink to="/admin/exercises/new" class="admin-btn admin-btn--primary">+ 新建练习</NuxtLink>
    </div>
    ...
    <table v-else class="admin-table">
      ...
      <tbody>
        <tr v-if="list.length === 0"><td colspan="6" class="admin-table__empty">暂无练习</td></tr>
        <tr v-for="(row, i) in list" :key="String(row.id ?? row.slug ?? i)">
          ...
          <td class="admin-table__actions">
            <NuxtLink :to="`/admin/exercises/${encodeURIComponent(String(row.slug))}`" class="admin-link">编辑</NuxtLink>
...
useHead({ title: '练习管理 · Admin' })
const { listExercises, removeResource } = useAdmin()
...
async function onDelete(row: Record<string, unknown>) {
  ...
  if (!slug) return
  if (!confirm(`确定删除练习：${String(row.title)} (${slug})？`)) return
  const r = await removeResource('exercise', slug)
