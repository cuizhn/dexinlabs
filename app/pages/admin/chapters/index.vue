  ...
  <div class="admin-list">
    <div class="admin-list__head">
      <div>
        ...
        <h1 class="admin-list__title">章节管理 Chapter</h1>
      </div>
      <NuxtLink to="/admin/chapters/new" class="admin-btn admin-btn--primary">+ 新建章节</NuxtLink>
    </div>
    ...
    <table v-else class="admin-table">
      <thead>
        <tr><th>标题</th><th>slug</th><th>course</th><th>排序</th><th>更新</th><th></th></tr>
      </thead>
      <tbody>
        <tr v-if="list.length === 0"><td colspan="6" class="admin-table__empty">暂无章节</td></tr>
        <tr v-for="(row, i) in list" :key="String(row.id ?? row.slug ?? i)">
          ...
          <td><code>{{ String(row.course || '—') }}</code></td>
          <td>{{ Number(row.order ?? 0) }}</td>
          <td>{{ fmt(row.updatedAt) }}</td>
          <td class="admin-table__actions">
            <NuxtLink :to="`/admin/chapters/${encodeURIComponent(String(row.slug))}`" class="admin-link">编辑</NuxtLink>
...
useHead({ title: '章节管理 · Admin' })
const { listChapters, removeResource } = useAdmin()
...
async function onDelete(row: Record<string, unknown>) {
  ...
  if (!slug) return
  if (!confirm(`确定删除章节：${String(row.title)} (${slug})？`)) return
  const r = await removeResource('chapter', slug)
