...
export default defineEventHandler(async (event) => {
  ...
  //   例如请求路径 /api/lesson/python-intro，则 slug 值为 "python-intro"
  ...
  //   queryCollection(event, 'lesson') 第一个参数传入 event 用于获取请求上下文（如数据库连接、用户鉴权信息等）
  //   第二个参数 'lesson' 指定要查询的集合/表名，返回值为查询构建器实例（QueryBuilder）
  const lesson = await queryCollection(event, 'lesson')
    ...
    //   返回数据结构：Promise<Lesson | null>
    //     找到时：解析为 Lesson 对象，包含该条 lesson 记录的全部字段（如 id, slug, title, content, created_at 等）
    //     未找到时：解析为 null（不是空数组）
    .first()

  // 错误处理：查询结果为空，说明数据库中不存在该 slug 对应的 lesson 记录
  if (!lesson) {
    // 抛出 404 Not Found 错误，在 statusMessage 中回显用户请求的 slug 便于调试定位
    //   响应体结构示例：{ statusCode: 404, statusMessage: 'Lesson not found: python-intro' }
    throw createError({
      statusCode: 404,
