...
export default defineEventHandler(async event => {
  try {
    ...
    const result = await exerciseService.remove(slug)
    const rowCount = (result && typeof (result as { rowCount?: unknown }).rowCount === 'number')
      ? Number((result as { rowCount: number }).rowCount)
      : null
    if (rowCount === 0) {
      throw createError({ statusCode: 404, statusMessage: `Exercise not found: ${slug}` })
    }
